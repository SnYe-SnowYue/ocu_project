...
def add_stock():
    ...
    stock_code = (request.form.get("stock_code") or "").strip().upper()
    ...
    try:
        ...
        with db.cursor() as cursor:
            cursor.execute(
                "SELECT ticker FROM etf_tickers WHERE ticker = %s LIMIT 1",
                (stock_code,),
            )
            if not cursor.fetchone():
                flash("請輸入有效 ETF 代號（例如：0050、0056、00878）。", "danger")
                return redirect(url_for("portfolio.list_stocks"))
