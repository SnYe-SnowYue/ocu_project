...
from .services.market_data import get_etf_snapshot, get_price_position, get_top_holdings_weights
...
def _register_routes(app: Flask) -> None:
    ...
    def home():
        ...
        try:
            ...
            with db.cursor() as cursor:
                ...
                for item in my_tickers:
                    ...
                    try:
                        holdings = get_top_holdings_weights(item["ticker_yfinance"])
                        for stock_en, weight in holdings.items():
                            sector_name = sector_lookup.get(stock_en, "其他")
                            all_sector_dist[sector_name] = (
                                all_sector_dist.get(sector_name, 0.0) + weight
                            )
                    except Exception:
                        continue
        ...
        dashboard_sector_analysis = sorted(
            [
                {"label": sector_name, "value": round(weight, 2)}
                for sector_name, weight in all_sector_dist.items()
            ],
